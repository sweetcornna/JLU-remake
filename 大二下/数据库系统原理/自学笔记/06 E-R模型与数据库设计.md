...
![自环联系集 works_for：employee 用两条线连到 works_for，上面一条标角色 manager 并带箭头指向 employee，下面一条标角色 worker；旁注一对多，一个 manager 可以管多个 worker](img/06-1_自环联系works_for的角色.png)
- 联系的属性（描述性属性）：联系也可以有属性，比如选课联系上的选课日期、成绩，`depositor`（顾客存款）联系上的访问日期 `access_date`。

![depositor 联系集的实例：左边五个顾客，右边六个账户，中间椭圆里每条连线对应一个联系，线上的日期就是这个联系的 access_date；Jones、Smith、Jackson 各连了两个账户](img/06-1_depositor联系实例及属性access_date.png)

图上方的联系集名印作 cdepositor，多了一个 c，应读作 `depositor`。
...
![复合属性的层次：name 分成 first_name、middle_initial、last_name；address 分成 street、city、state、postal_code，其中 street 又分成 street_number、street_name、apartment_number](img/06-1_复合属性name和address的层次.png)
...
![从左到右依次是一对一、一对多、多对一、多对多：一对一时 a1 到 a4 各连一个 b；一对多时 a1 连 b1 和 b2，a2 连 b3 和 b4，a3 连 b5；多对一时 a1 到 a5 每个只连一个 b，b1 和第二个 b 各被两三个 a 连着；多对多时 a1、a2 各连两个 b，b2、b4 也各被两个 a 连着](img/06-2_四种映射基数的实体对应示意.png)

笔记原图的多对一那张，第二个 a 和第二个 b 漏印了下标，按顺序应读作 a2 和 b2。
...
![customer 到 borrower 是单线，标部分参与；borrower 到 loan 是双线，标全部参与；旁注部分顾客可能参与贷款，但贷款信息必须对应顾客](img/06-2_customer部分参与loan全部参与borrower.png)
...
| 多对一（见下一张图） | 箭头指向 `student`，`instructor` 一侧是无向线段 | 1 个老师只能指导 1 个学生，1 个学生可以由多个老师指导 |
| (c) 多对多 | 两侧都是无向线段 | 1 个老师可以指导多个学生，1 个学生可以由多个老师指导 |

![advisor 的多对一画法：instructor 一侧是无向线段，箭头从 advisor 指向 student](img/06-3_老师资料advisor多对一画法.png)
...
笔记里的多元联系例子是三元联系 `works_on`，把员工、分行和工作三个实体集连在一起，三条边都没有箭头：

![三元联系 works_on：employee、job、branch 三个实体集各用一条无箭头的线连到同一个菱形 works_on，旁注多元联系](img/06-4_三元联系works_on.png)
...
![loan 画成 customer 和 branch 之间的菱形，loan_number 和金额两个椭圆挂在菱形上；金额那个椭圆印作 account，应为 amount](img/06-4_把loan画成联系集.png)
...
![弱实体集 payment 画成双矩形，分辨符 payment_number 加虚下划线，经双菱形 loan_payment 用箭头指向 loan，payment 一侧是双线；旁注弱实体集与标识实体集的联系集必然多对一，弱实体集的实体全部参与](img/06-5_弱实体集payment依赖loan.png)
...
笔记里表格前两行各配了一张图。`borrower` 两侧都没有箭头，是多对多，单独建模式；`loan` 的第二个属性在图里又印成了 account，应为 amount。

![customer 与 loan 经菱形 borrower 相连，两侧都没有箭头；下方写出 borrower 等于 customer_id 和 loan_number，两者都加下划线](img/06-7_多对多borrower单独建模式.png)

`account_branch` 从 `account` 一侧是双线（每个账户都属于某个支行），另一侧箭头指向 `branch`，是多对一，所以不单独建模式，把 `branch_name` 并进 `account`。

![account 用双线连到菱形 account_branch，再用箭头指向 branch；下方写出 account 等于 account_number、branch_name、balance，只有 account_number 加下划线](img/06-7_多对一account_branch并入account.png)
...
![customer 的复合属性 customer_name 下挂 first_name、middle_initial、last_name；下方的 customer 模式里 customer_name 不见了，三个子属性和 customer_street、customer_city 并列，红笔旁注同其他属性有同等地位](img/06-7_复合属性customer_name拆成子属性.png)

![customer 带一个双椭圆的多值属性 telephone_number；下方写出单独的模式 customer_phone 等于 customer_id 和 telephone_number](img/06-7_多值属性telephone_number单独建模式.png)

（更正：多值属性那张图的 `customer_phone` 只在 `customer_id` 下画了下划线。一个顾客可以有好几个电话，`customer_id` 单独当主码会重复，`telephone_number` 也要加下划线，两者一起作主码。）
...
![笔记里的四张 borrower 图，从上到下：两侧无箭头；箭头指向 customer，旁注有箭头代表一、本图即为一对多；箭头指向 loan；两侧都有箭头但标题写成多对多。loan 的第二个属性印作 account，应为 amount](img/06-题2_borrower四种映射基数的E-R图.png)
