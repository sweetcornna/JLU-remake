...
课件那张图把两种做法画在一起，手写分别标了 Jackknife 和 Bootstrap：

- **Jackknife**：从总体里取一个样本，每次剔掉一个观测，得到一个估计量 $\hat\theta$（图上只画了取样本、得估计量这一步，见下面的说明）。
- **Bootstrap**：从样本里反复抽出子样本，每个子样本给出一个估计量 $\hat\theta_1^*,\hat\theta_2^*,\dots$。

图上有一条虚线从「总体」指向「样本」，意思就是 Bootstrap 把**样本当成总体**来抽。这是整个方法的出发点。

![左右两幅示意图：左边是总体中圈出一个样本，箭头指向唯一的估计量 θ̂；虚线把这个样本连到右边，右边以样本为大圈，从中反复抽出子样本 1、子样本 2 等，各自得到估计量 θ̂1*、θ̂2*](图/3-7-1_Jackknife与Bootstrap对比.png)

读这张图要注意左半边：手写在上面标的是「Jackknife」，但图里画的是从总体抽一个样本、算出一个估计量 $\hat\theta$，并没有画「每次剔掉一个观测」的子样本，实际是传统的一次抽样估计。把它当成「传统做法」看更准确：传统做法只有一个 $\hat\theta$，看不出它的波动；右半边的 Bootstrap 从同一个样本里有放回地抽出很多子样本，得到一串 $\hat\theta^*_1,\hat\theta^*_2,\dots$，这串数的离散程度就是 $\hat\theta$ 波动的估计。上面列的 Jackknife 定义本身没错，只是这张图没画它。
