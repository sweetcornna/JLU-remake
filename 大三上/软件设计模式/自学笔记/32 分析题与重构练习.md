...
这道题是 2018 级期末卷的类图分析题第 1 题，原卷的图 a、图 b：

![图 a 是接口 IClient 同时声明 connect、disconnect、receive、send，由 Client 实现；图 b 拆成连接接口 IClient1 和传递接口 IClient2，分别由 Client1 和 Client2 实现](图/2018级_单一职责图a图b.png)
...
（更正：文件B p2 转载的图 b 两个实现类都标成了 Client2，按图意应为 Client1 和 Client2，扫描卷上的原图就是这样标的。）
...
![Employee 初始类图，七个私有属性和按天、按时计薪及显示工资三个方法挤在一个类里](图/补充_分析题2_Employee初始类图.png)
...
![Button、ComboBox、TextBox、List、Label 五个组件类互相以聚合关系持有引用，连线呈网状](图/补充_分析题3_组件网状引用类图.png)
...
![Client 用虚线依赖箭头分别指向 Circle、Triangle、Rectangle 三个图形类，每个类都有 init、setColor、fill、setSize、display 五个方法](图/补充_分析题4_图形库初始类图.png)
