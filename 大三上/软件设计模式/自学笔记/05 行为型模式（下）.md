...
换成通用角色名，标准类图是两条继承线加一个观察者集合：

![观察者标准类图：ConcreteSubject 继承抽象的 Subject，Subject 用 observers 列表带空心菱形聚合 1 到多个 Observer；ConcreteObserver 实现 Observer 接口的 update，并用实线箭头通过 subject 关联 ConcreteSubject 以读取 subjectState](图/36_观察者结构.png)
...
![MVC 三者的关系：用户操作从 View 传到 Controller，Controller 改 Model 的数据、也可以换 View 的界面，Model 的数据变了就通知 View，这条通知线就是观察者模式](图/36_MVC.png)
...
![猫鼠狗类图，按文件B 7.1 的写法：Cat 继承抽象类 MySubject，MySubject 用 observers 列表带空心菱形聚合 1 到多个 MyObserver，Mouse 和 Dog 实现 MyObserver 接口的 response](图/36_猫鼠狗实例.png)

图里没有主人 Master，它和 Mouse、Dog 一样实现 MyObserver，挂到同一个接口下就行。下面的代码用接口 Subject 代替了抽象类 MySubject。
...
![状态标准类图：Context 持有 state 字段，用实线箭头关联抽象的 State，提供 request 和 setState；ConcreteStateA、ConcreteStateB 继承 State 实现 handle](图/37_状态结构.png)

这张标准图只画了环境类到状态的单向关联，对应"环境类负责转换"的写法。状态类自己负责切换时，要再从 State 画一条指回 Context 的依赖或关联线。
...
![论坛用户等级类图：ForumAccount 用 state 字段实线关联抽象类 AbstractState，AbstractState 用 acc 字段关联回 ForumAccount，两者双向关联；PrimaryState 只覆盖 downloadFile，MiddleState 只覆盖 writeNote，HighState 覆盖 writeNote 和 downloadFile](图/37_论坛等级实例.png)

这张图和下面的代码有两处不一样。图里积分 `point` 画在 ForumAccount 里，环境类的下载方法叫 `download(int)`；代码按文件B 的写法把积分放在状态对象里，方法名统一成 `downloadFile(int)`。两种放法都能工作，答题时类图和代码保持一致即可。图里各具体状态也没画出 `checkState()`，实际每个具体状态都要实现它。
...
![银行账户状态类图：Account 用 state 字段实线关联抽象类 AccountState，AccountState 用 acc 字段关联回 Account；NormalState 和 OverdraftState 覆盖 computeInterest，RestrictedState 覆盖 withdraw 和 computeInterest](图/37_银行账户实例.png)

骨架和上一题论坛用户等级那张图相同：环境类和抽象状态双向关联，具体状态只覆盖自己和默认行为不同的方法。
...
![TCP 连接状态类图：TCPConnection 用 state 字段实线关联 TCPState 接口，接口声明 open、close、acknowledge，TCPEstablished、TCPListening、TCPClosed 三个类实现它](图/37_TCP连接.png)

这张图出自讲义引入状态模式时的示意，类名写法和下面的代码略有不同（TCPState 对应 TcpState，TCPClosed 对应 TcpClosedState），三个具体状态框里只写了 `open()` 作代表，实际三个方法都要实现。
...
![策略标准类图：Context 带空心菱形通过 strategy 字段聚合抽象的 Strategy，提供 setStrategy 和 algorithm；ConcreteStrategyA、ConcreteStrategyB 继承 Strategy 实现 algorithm](图/38_策略结构.png)
...
![排序策略类图：ArrayHandler 带空心菱形通过 sortObj 字段聚合抽象的 Sort，提供 setSortObj 和 sort；BubbleSort、SelectionSort、InsertionSort 继承 Sort 实现 sort](图/38_排序策略实例.png)
...
换成通用角色名的标准类图，三类基本方法在父类里用可见性和标注区分：

![模板方法标准类图：抽象类 AbstractClass 有 public 的 templateMethod 和具体方法 primitiveOperation1，protected 的抽象方法 primitiveOperation2 和钩子方法 primitiveOperation3；ConcreteClassA 覆盖 primitiveOperation2 和钩子 primitiveOperation3，ConcreteClassB 只实现 primitiveOperation2](图/39_模板方法结构.png)
...
- 在模板方法模式里的体现：子类只负责覆盖父类的方法、提供具体步骤，不去显式调用父类；什么时候调用子类的方法由父类的模板方法决定，父类控制整个过程。
  ...
  ![模板方法的调用方向：客户端只调用父类的 templateMethod 这一句，再由父类反过来调用子类覆盖的基本方法](图/39_反向控制.png)
...
![银行业务类图：抽象类 BankTemplateMethod 有模板方法 process、具体方法 takeNumber 和 evaluate、抽象方法 transact，Deposit、Withdraw、Transfer 继承它，各自只实现 transact](图/39_银行业务实例.png)
...
换成通用角色名，两个层次结构只在 `accept()` 处交汇：

![访问者标准类图：ConcreteElementA、ConcreteElementB 实现 Element 接口的 accept，Element 以 Visitor 作为 accept 的参数虚线依赖 Visitor；ConcreteVisitorA、ConcreteVisitorB 继承 Visitor，各自为两种具体元素实现 visit；ObjectStructure 带空心菱形聚合 Element，提供 accept 和 addElement](图/3A_访问者结构.png)
