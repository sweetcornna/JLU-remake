...
创建型的六个模式可以按下面的顺序逐个排除：

![创建型模式选择流程：依次问只能有一个实例吗、装配过程复杂多变吗、类型运行期才定吗、产品要成族配套吗、以后要加新产品吗，前四问答是分别选单例、建造者、原型、抽象工厂，最后一问答是选工厂方法，答否选简单工厂](图/90_创建型模式选择流程.png)

图的最后一步画得容易看错：标着"否"的那根箭头是从最后一个菱形"以后要加新产品"引出来的，指向简单工厂，并不是从工厂方法走到简单工厂。
...
代理和装饰最难分，代码几乎一样。把两张类图放在一起看：

![装饰模式简图：ConcreteComponent 和抽象的 Decorator 都继承抽象类 Component，Decorator 用 protected 字段 component 以空心菱形聚合 Component](图/90_装饰简图.png)

![代理模式简图：RealSubject 和 Proxy 都实现 Subject 接口，Proxy 用 real 字段实线关联具体的 RealSubject](图/90_代理简图.png)

两张图都是"和被包装者实现同一个接口，再持有它"。差别在连线：装饰器聚合的是抽象的 Component，可以一层套一层；代理关联的是具体的 RealSubject，通常只包一层。区分还要看两点：
...
![策略标准类图：Context 带空心菱形通过 strategy 字段聚合抽象的 Strategy，ConcreteStrategyA、ConcreteStrategyB 继承 Strategy 实现 algorithm](图/38_策略结构.png)

![桥接标准类图：抽象类 Abstraction 用 protected 字段 impl 以空心菱形聚合 Implementor 接口，RefinedAbstraction 继承 Abstraction，ConcreteImplementorA、ConcreteImplementorB 实现 Implementor](图/21_桥接结构.png)

两张图唯一的结构差别是左边那一侧：桥接的 Abstraction 下面还有 RefinedAbstraction，左右两边都在长子类；策略的 Context 一般不派生子类。
