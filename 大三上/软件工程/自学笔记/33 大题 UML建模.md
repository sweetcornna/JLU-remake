...
![报告紧急情况的用例规约：用例名称、参与者、入口条件、事件流、出口条件五栏，事件流写了第 2 到第 4 步](images/33_报告紧急情况用例规约.png)
...
![报告紧急情况的手绘顺序图：生命线从左到右是现场人员、终端、事故管理系统、数据库、界面、调度员，消息 1 激活到 13 确认，其中打开事件和发给现场人员的确认用红色虚线补画](images/33_报告紧急情况顺序图手写答案.png)
...
![图3-1 书籍销售系统用例图：Customer 连 U1、U2、U3，U2 以 extend 指向 U3；A3 连 Produce picklist；A1 连 Promote books、Add books、Reorder books；A2 连 Update stock](images/2017级_书籍销售用例图.png)

图 3-1 用例图转写（图中没有画系统边界）：
...
![图3-2 书籍销售系统初始类图：C1 有 name、address、email，指向 C2；C2 聚合 OrderedBook，OrderedBook 有 quantity 并指向 C3；C3 有 title、author、introduction，Catalog 聚合 C3](images/2017级_书籍销售初始类图.png)
