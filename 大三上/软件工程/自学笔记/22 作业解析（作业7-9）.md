...
- 这三次作业几乎每题都带图。题图已放进正文，图下另用表格或 mermaid 代码块转写一遍，方便抄写和对照。
...
![作业7第2题的工程网络图：事件 1 到 6，边上标 A:3、B:6、C:5、D:3、E:4、F:5、G:3](images/作业7-2_六事件工程网络图.png)
...
![作业7第19题题图：13 个事件 A 到 M，A 标 START、M 标 FINISH，箭头上只标持续时间](images/作业7-19_工程网络图题图.png)

图中共 13 个事件（圆圈 A 到 M，A 为 START，M 为 FINISH），箭头上的数字是持续时间：
...
![作业8第17题用例图：参与者 X1、X3 都泛化到 X2；X2 关联 UC4，X1 关联 UC3；UC4 以 include 指向 UC5，UC2 以 extend 指向 UC4；UC4 和 UC1 都泛化到 UC3](images/作业8-17_用例图.png)
...
![作业8第18题初始类图 1-1：A 和 B 之间有编写、演奏两条关联；C、D 继承 A，C 端空心菱形聚合 D；E 端空心菱形聚合 B；F 端实心菱形组合 E；六个括号是待填的多重度](images/作业8-18_唱片初始类图.png)
...
![作业8第19题：上面是 ORS 系统的用例图，A2、A3 泛化到 A1，A4 泛化到 A3，关系（1）（2）待填；下面是图 2 提交稿件过程的活动图，Action1 到 Action4 待填](images/作业8-19_ORS用例图和活动图.png)

原图把 set preferences 拼成了 set perferences，按表2 的写法作答。
...
![作业8第20题图1 ATM 系统顶层用例图：Operator 关联系统启动和停止；A1 关联 U1，A2 关联 U3；U1 以 include 指向 U3，U2 以关系（1）指向 U3；Inquire、Transfer、Withdraw、Deposit 泛化到 U3](images/作业8-20_ATM顶层用例图.png)
...
![作业8第20题图2 一次会话的序列图：消息 1 到 11 依次在读卡器、ATM、会话、客户控制台、事务之间传递，6 到 9 待填，8 和 9 在 loop 框里](images/作业8-20_一次会话序列图.png)
...
![作业8第24题组件图：AccountManagement 向下伸出两个需要接口的半圆插口，分别套在 CreditCardServices 提供的 IdentityVerifier 和 Logger 提供的 TransactionLogger 上](images/作业8-24_组件图.png)
...
![作业9第1题顺序图：Customer 向 FundsTransactionManager 发 start，管理器向 source:Account 发 checkBalance 和 withdraw，向 target:Account 发 deposit，3 号消息 Balance 是虚线返回](images/作业9-1_转账顺序图.png)
...
![三种分析类图标：a 是顶上带小箭头的圆，b 是底下贴一条横线的圆，c 是左边连一根竖线的圆](images/作业9-3_分析类图标.png)
...
![作业9第6题的部分顺序图：会员向检索零件UI 发提交查询条件和选中零件，检索零件UI 向右侧被截断的对象发检索零件和取零件信息，并两次自调用显示结果](images/作业9-6_检索零件顺序图.png)
...
![作业9第7题原设计类图：UserUI 指向 AccountManagement，AccountManagement 持有 SilverAcct 和 GoldenAcct 两个具体类，两个账户类的属性和方法完全相同](images/作业9-7_乘客账户原设计类图.png)
...
![作业9第8题原设计类图：Client 指向 MarineLife，MarineLife 用四条聚合分别持有 Tuna、Salmon、Abalone、Scallop，四个类各有一个 show 方法](images/作业9-8_海洋生物原设计类图.png)
...
![策略模式通用结构：Context 以名为 strategy 的聚合持有抽象类 Strategy，三个具体策略类继承 Strategy 并实现 AlgorithmInterface](images/作业9-9_策略模式通用结构.png)
...
![飞机模拟系统的策略模式类图：AirCraft 用两条聚合持有 FlyBehavior 和 TakeOffBehavior，两个行为类各有两个具体子类，四种飞机继承 AirCraft](images/作业9-9_飞机模拟策略模式类图.png)
...
![作业9第10题顺序图：Prospective Buyer、边界类 PersonalPlannerForm、控制类 PersonalPlannerController、实体类 BuyerRecord 和 PlannerProfile 五条生命线，上半段维护档案，下半段提交档案信息](images/作业9-10_个人规划顺序图.png)
