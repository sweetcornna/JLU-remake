...
整理自 `并行与分布式笔记.pdf` 第 22–67 页（第 2 章 线程级并行：OpenMP 编程）。原稿里仍能渲染出来的结构图、示意图都已裁出放进正文，放在 `图/` 目录。
...
两者都有**统一的地址空间**，都属于共享内存模型，本章只讨论这种情况。原稿标注这两种访存模型「可以介绍并画图对比」：

![UMA 与 NUMA 对比：左边 UMA 的 CPU 1 到 CPU N 都经总线访问同一块 Memory；右边 NUMA 的每个 CPU 旁边各挂一块 Memory 1 到 Memory N，CPU 之间再经总线互连](图/2-1_UMA与NUMA对比.png)

典型的 UMA 是集中式共享存储器 SMP，每个处理器带一或多级专用缓存，往下接共享缓存、主存储器和 I/O 系统：

![SMP 结构：四个处理器各带一或多级专用缓存，汇到同一个共享缓存，再接主存储器和 I/O 系统](图/2-1_SMP集中式共享存储.png)

典型的 NUMA 是分布式共享存储器 DSM，每个处理器 P 各带自己的存储器 M 和 I/O，所有节点挂在一张互连网络上：

![DSM 结构：上下各四个结点，每个结点由处理器 P、存储器 M 和 I/O 组成，全部接到中间的互连网络](图/2-1_DSM分布式共享存储.png)
...
原稿要求「画图并描述 core、die、package 和 socket 之间的关系」，配的是一组实物照片：

![从主板到核心的逐级放大：主板上红圈标出的 socket 插着 package；package 上较大的那颗芯片放大后是 die，die 上有四个 Sunny Cove Core、LLC、System Agent 和 Gen11 GPU；再放大其中一个 core，能看到执行单元、L1 数据与指令缓存、L2 和 LLC 切片](图/2-1_Core_Die_Package与Socket.png)
...
并发与并行的区别：并发是 core 0 一个核上轮流跑 A0、B0、A1、B1、A2、B2；并行是 core 0 跑 A0、A1、A2，core 1 同时跑 B0、B1、B2，对应 thread 0 和 thread 1。

![并发与并行：左边只有 core 0，A0、B0、A1、B1、A2、B2 交替排成一列；右边 core 0 和 core 1 两列，A 组和 B 组各占一列同时执行，下方分别引出 thread 0 和 thread 1](图/2-1_并发与并行.png)
...
- 左侧处理器执行 `A = 1`，把值 1 写入缓存，更新经互连网络发往其他处理器。
- 中间处理器很快收到了 `A = 1`（它的缓存里 `A: 0 → 1`），于是等待条件 `A == 1` 成立，接着执行 `B = 1`。
- `B = 1` 很快传到右侧处理器，但左侧发往右侧的 `A = 1` 在网络里走了弯路（图中的锯齿线），还没到。右侧处理器看到 `B == 1` 就去读 `A`，缓存里还是旧的 `A = 0`，出现 `Oops!` 错误。

（更正：原文把第二步写成「中间处理器会误认为 A 的值仍为 0」。按原图，中间处理器缓存里的 A 已经从 0 变成 1，读到旧值 0 的是右侧处理器。）

![缓存导致的可见性问题：三个处理器各带 Cache 和 Memory，经互连网络相连；左侧写 A = 1，中间等到 A == 1 后写 B = 1，右侧等到 B == 1 后读 A；右侧缓存里 B 已是 1 而 A 仍是 0，因为发往右侧的 A = 1 在网络中延迟](图/2-2_缓存导致的可见性问题.png)
...
![互联网络导致的可见性问题：左侧处理器先写 A = 1 再写 Ready = 1；A 所在的存储器在中间结点，那条更新在网络中延迟，存储器里 A 仍是 0；Ready 所在的右侧存储器已从 0 变成 1，右侧处理器等到 Ready == 1 后读 A，读到的是 0](图/2-2_互联网络导致的可见性问题.png)
...
![OpenMP 存储模型：thread 0 和 thread 1 各自连着一块 Private，中间共同连着一块 Shared](图/2-3_OpenMP存储模型.png)
...
应用通过编译制导、用户通过环境变量，都要落到运行库例程上，运行库例程再去用 OS 线程。图上应用还有一条长箭头越过编译制导直通运行库例程，对应程序里直接调用 `omp_get_thread_num()` 这类库函数。

![OpenMP 体系结构：四层方框，最上层左为应用、右为用户；第二层左为编译制导、右为环境变量；第三层是运行库例程；最下层是 OS 线程；应用经编译制导、用户经环境变量到达运行库例程，应用另有一条箭头直达运行库例程](图/2-3_OpenMP体系结构.png)
...
![并行域结构图：竖直时间轴上 Master thread 串行执行，遇到 !$OMP PARALLEL 分出一排 Threads 并行执行，到 !$OMP END PARALLEL 处的 barrier 汇合，回到 Master thread；之后第二个并行域重复一次](图/2-5_并行域结构.png)

图右边的 `!$OMP PARALLEL` 是 Fortran 的写法，对应 C 里的 `#pragma omp parallel`。
...
![三种任务划分结构：主线程经 FORK 分出线程列，分别进入 DO/for loop（每个线程分到一部分迭代）、SECTIONS（每个线程执行长短不一的不同代码段）、SINGLE（只有一个线程执行，其余线程直接到 JOIN 等待），最后都在 JOIN 汇合回主线程](图/2-6_任务划分结构.png)
...
![下三角矩阵乘向量：6 行的下三角矩阵 L 乘向量 Y，结果第一行是 L11 乘 Y1，第二行是 L21 乘 Y1 加 L22 乘 Y2；左侧色块表示前 3 行分给一个线程、后 3 行分给另一个线程](图/2-6_下三角矩阵乘向量.png)

按图上两个线程各分 3 行来数：前 3 行一共 1 + 2 + 3 = 6 次乘加，后 3 行一共 4 + 5 + 6 = 15 次，第二个线程的活是第一个的 2.5 倍。
...
线程数为 4、迭代 1 到 40 时，两种 static 的分法：

![static 调度分块：schedule static 把 1 到 40 平均切成四段依次给 T0 到 T3；schedule static 4 把 1 到 40 切成 10 个宽度为 4 的块，按 T0 T1 T2 T3 T0 T1 T2 T3 T0 T1 轮转](图/2-6_schedule_static分块.png)

图上把 schedule 拼成了 schudule，是原图的笔误。10 个块轮转下来，T0 和 T1 各拿 3 块，T2 和 T3 各拿 2 块。
